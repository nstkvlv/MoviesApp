...
final class MovieViewController: UIViewController {
    ...
    private func setupMovieCollectionView() {
        if let layout = movieCollectionView?.collectionViewLayout as? CustomLayout {
          layout.delegate = self
        }
        movieCollectionView?.contentInset = UIEdgeInsets(top: 23, left: 5, bottom: 10, right: 5)
        movieCollectionView.delegate = self
        movieCollectionView.dataSource = self
        movieCollectionView.register(UINib(nibName: "PopularMovieCell", bundle: nil),
...
extension MovieViewController: CustomLayoutDelegate {
    func collectionView(_ collectionView: UICollectionView, heightForPhotoAtIndexPath indexPath:IndexPath) -> CGFloat {
        return CGFloat(viewModel.sizes.randomElement()!)
    }
}
